...
TOOL_IDS = [
    "t56eec602ed5e",  # query_database
    "t4f88b252178e"   # search_documents
]
...
def main():
    ...
    print("Migrating AGIX companion personas to NEW Tavus account...\n")
    ...
    for agent in AGENTS:
        ...
        if res.status_code not in [200, 201]:
            print(f"  [Error] Failed to create {name}: {res.text}")
            continue
            
        persona_id = res.json().get("persona_id")
        print(f"  [Success] Created {name} -> {persona_id}")
        
        # 2. Attach magic_canvas skill
        ...
        # 4. Update Database
        # Extract base name to match in DB (e.g., "Aria Native Study Companion " -> "Aria")
        base_name = name.split(" Native")[0]
        companion = db.query(Companion).filter(Companion.name.ilike(f"%{base_name}%")).first()
        if companion:
            old_id = companion.tavus_persona_id
            companion.tavus_persona_id = persona_id
            db.commit()
            print(f"  [DB Updated] {base_name}: {old_id} -> {persona_id}")
        else:
            print(f"  [DB Error] Could not find Companion in DB for {base_name}")
            
        print("-" * 50)

    db.close()
    print("\nAll done! You can now restart your sessions.")
